...
class TrustedShop {
  ...
  public function __construct($WebApiKey, $serviceUrl) {
  ...
  public function Prepare() {
    ...
    $Output.= 'var I = new Image(); I.src="' . $this->serviceUrl . self::SERVICE_TOKEN_PROCESS . $Query . '" + c;';
    $Output.= '};</script>';
    // Include:
    ...
    // Fallback:
    $Output.= '<noscript>';
    $Output.= '<img src="' . $this->serviceUrl . self::SERVICE_TOKEN_PROCESS . $Query . $Random . '" />';
    $Output.= '</noscript>';
  ...
  protected function GetQuery($Params) {
    ...
    $Curl = curl_init();
    curl_setopt($Curl, CURLOPT_URL, $this->serviceUrl . self::SERVICE_TOKEN_REQUEST);
