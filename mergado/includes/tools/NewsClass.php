...
class NewsClass
{
    ...
    public static function saveArticle(array $item, DateTime $date, $lang)
    {
        ...
        Db::getInstance()->insert(Mergado::MERGADO['TABLE_NEWS_NAME'], [
    ...
    {
        Db::getInstance()->update(Mergado::MERGADO['TABLE_NEWS_NAME'], ['shown' => 1], '`id` IN (' . implode(',', $ids) . ')');
    }
    ...
    public static function areNewsHidden()
    {
        $newsHiddenUntilTimestamp = SettingsClass::getSettings(SettingsClass::COOKIE_NEWS, 0);

        if($newsHiddenUntilTimestamp != 0) {
            $newsHiddenUntil = new DateTime($newsHiddenUntilTimestamp);
        } else {
            $newsHiddenUntil = new DateTime();
        }

        if ($newsHiddenUntil <= new DateTime()) {
            return false;
        } else {
            return true;
        }
