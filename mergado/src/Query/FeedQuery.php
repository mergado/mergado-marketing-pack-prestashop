...
class FeedQuery
{
    ...
    public function productsToFlat($start, $limit, $currency, $lang = false, $export_out_of_stock = false): array
    {
        $flatProductList = [];
        ...
        $export_both = DatabaseManager::getSettingsFromCache(SettingsService::EXPORT['BOTH']);
        $export_none = DatabaseManager::getSettingsFromCache(SettingsService::EXPORT['NONE']);
        $export_catalog = DatabaseManager::getSettingsFromCache(SettingsService::EXPORT['CATALOG']);
        $export_search = DatabaseManager::getSettingsFromCache(SettingsService::EXPORT['SEARCH']);
        $export_cost = DatabaseManager::getSettingsFromCache(SettingsService::EXPORT['COST']);
        ...
        foreach ($productsList as $productCore) {
            ...
            $base = $this->productBase($product, $lang, $currency, $export_cost, $export_out_of_stock);
    ...
    public function productBase($item, $lang, $currency, $export_cost = true, $export_out_of_stock = false): ?array
    {
        ...
        $whenOutOfStock = ProductHelper::getStockStatusLogic($item->id);
        ...
        if (!empty($combinations)) {
            foreach ($combinations as $combination) {
                ...
                $saleDateInterval = $this->getSaleDateInterval($currencyId,$id_country, $context->shop->id, $combination['id_product'], $combination['id_product_attribute']);
                ...
                //$price = Tools::convertPriceFull($price, $this->defaultCurrency, $this->currency);
                if ($mainImage === NULL) {
                ...
                $productBase[] = [
                    ...
                    'availability' => ProductHelper::getProductStockStatus($combination['id_product'], $combination['id_product_attribute']),
                    'stock_quantity' => $qty,
                    'category' => $category,
...
//                    'url' => $link->getProductLink($item, null, $defaultCategoryName, null, $lang, null, $combination['id_product_attribute'], false, false, true),
            ...
            if ($qty <= 0 && $whenOutOfStock == 0 && !$export_out_of_stock) {
                ...
                $saleDateInterval = $this->getSaleDateInterval($currencyId,$id_country, $context->shop->id, $item->id, null);
                ...
                $productBase = [
                    ...
                    'itemgroup_id' => $itemgroupBase,
                    'accessory' => $accessoriesExtended,
                    'availability' => ProductHelper::getProductStockStatus($item->id),
    ...
    {
        //Count product cost
//        if(DatabaseManager::getSettings(SettingsService::EXPORT['COST'],  $shopId)) {
...
//        }
    ...
    public function getProductCombination($product, $lang): array
    {
        $groups = [];
        $comb_array = [];
        $flatProductList = [];
                                ...
                                $active = true, $random = false, $random_number_products = 1, Context $context = null)
    {
        ...
        if (PrestashopVersionHelper::is16AndHigher()) {
     ...
     * @throws PrestaShopException
     */
    private function getProductsProperties($id_lang, $query_result): array
    {
        $results_array = [];
    ...
    private function getProductProperties($id_lang, $row, Context $context = null)
    {
        ...
        return Product::getTaxesInformations($row, $context);
    }

    private function getPriceStatic($id_product, $usetax = true, $id_product_attribute = null, $decimals = 6, $divisor = null, $only_reduc = false, $usereduc = true, $quantity = 1, $force_associated_tax = false, $id_customer = null, $id_cart = null, $id_address = null, &$specific_price_output = null, $with_ecotax = true, $use_group_reduction = true, Context $context = null, $use_customer_price = true): float
    {
        if (!$context) {
            $context = Context::getContext();
        }
...
//     * 2 - use product information
        $productLableSettingsUsage = isset($product->additional_delivery_times) ? (int) $product->additional_delivery_times : 0;
        $deliveryDaysDefault = DatabaseManager::getSettingsFromCache('delivery_days');
        ...
        if (trim($deliveryDaysDefault) === '') {
            if ($availableQuantity > 0) {
                if ($productLableSettingsUsage === 0) {
                    ...
                    $deliveryDays = Configuration::get('PS_LABEL_DELIVERY_TIME_AVAILABLE', Configuration::get('PS_LANG_DEFAULT')); // global label
                ...
                if ($productLableSettingsUsage === 0) {
                    ...
                    $deliveryDays = Configuration::get('PS_LABEL_DELIVERY_TIME_OOSBOA', Configuration::get('PS_LANG_DEFAULT')); // global label
                } else if ( $productLableSettingsUsage === 2 ) {
    ...
    public function getProductPrice($currencyId, $idCountry, $specificPrice, $shopId, $productId, $productAttributeId = null, $with_vat = false)
    {
        return Product::priceCalculation(
            $shopId, // ID shop
    ...
    public function getProductDiscountPrice($currencyId, $idCountry, $specificPrice, $shopId, $productId, $productAttributeId = null, $with_vat = false)
    {
        return Product::priceCalculation(
            $shopId, // ID shop
            $productId, // ID Product
    ...
    public function getSaleDateInterval($currencyId, $id_country, $shopId, $productId, $productAttributeId = null)
    {
        $from = '';
        $to = '';
